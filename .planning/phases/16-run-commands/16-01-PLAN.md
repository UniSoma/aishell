...
must_haves:
  ...
  artifacts:
    - path: "src/aishell/config.clj"
      ...
      exports: ["load-config", "validate-config"]
  key_links:
...
Output: Working `config.clj` module with load-config and validate-config functions.
</objective>
...
```
  ...
  <done>
- config.clj exists with load-config and validate-config functions
...
<task type="auto">
  <name>Task 2: Test config loading with sample YAML</name>
  <files>src/aishell/config.clj</files>
  <action>
Test the config loading with real YAML to verify parsing works correctly.

1. Create a test config file temporarily:
```bash
mkdir -p /tmp/test-project/.aishell
cat > /tmp/test-project/.aishell/config.yaml << 'EOF'
mounts:
  - ~/.ssh
...
```bash
bb -e '
(require (quote [aishell.config :as config]))
(let [cfg (config/load-config "/tmp/test-project")]
  (println "mounts:" (:mounts cfg))
  (println "env:" (:env cfg))
  (println "ports:" (:ports cfg))
  (println "docker_args:" (:docker_args cfg))
  (println "pre_start:" (:pre_start cfg)))
'
```

Expected output:
- mounts: vector of strings
- env: map with nil values (passthrough) and actual values (literal)
- ports: vector of strings
- docker_args: string
- pre_start: string

3. Test unknown key warning:
```bash
cat > /tmp/test-project/.aishell/config.yaml << 'EOF'
mounts:
  - ~/.ssh
...
bb -e '(require (quote [aishell.config :as config])) (config/load-config "/tmp/test-project")' 2>&1 | grep -q "Unknown config keys"
```

4. Test invalid YAML (should error):
```bash
cat > /tmp/test-project/.aishell/config.yaml << 'EOF'
mounts: [
...
bb -e '(require (quote [aishell.config :as config])) (config/load-config "/tmp/test-project")' 2>&1
...
If tests fail, adjust config.clj to handle edge cases.
  </action>
  <verify>
```bash
cd /home/jonasrodrigues/projects/harness
