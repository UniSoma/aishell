...
must_haves:
  truths:
    ...
    - "Docker run args include --rm, -it, --init flags"
    ...
    - "PRE_START is passed via -e PRE_START=command"
  artifacts:
...
(defn build-docker-args
  ...
  (let [uid (get-uid)
    ...
    (-> ["docker" "run"
        ...
        ;; Config: pre_start (passed to entrypoint)
...
- Vector includes --rm, -it, --init flags
...
<task type="auto">
  <name>Task 3: Test complete argument building with config</name>
  <files>src/aishell/docker/run.clj</files>
  <action>
Test the full argument building with a complete config to verify all parts work together.

1. Test with full config:
```bash
bb -e '
(require (quote [aishell.docker.run :as run]))
...
- `-e PRE_START=echo hello`
- `--cap-add=SYS_PTRACE`
- `aishell:base` (last)

3. Test port validation (should error on invalid):
```bash
bb -e '
(require (quote [aishell.docker.run :as run]))
(run/build-docker-args
  {:project-dir "/tmp"
   :image-tag "test"
   :config {:ports ["invalid-port"]}
   :git-identity nil})
' 2>&1 | grep -q "Invalid port"
```

4. Test mount warning for non-existent source:
```bash
bb -e '
(require (quote [aishell.docker.run :as run]))
(run/build-docker-args
...
(let [args (run/build-docker-args
             {:project-dir (System/getProperty "user.dir")
              :image-tag "aishell:base"
              :config {:pre_start "redis-server"}
              :git-identity (run/read-git-identity ".")})]
  (assert (some #(= "--init" %) args) "Should have --init")
  (assert (some #(str/includes? % "PRE_START=") args) "Should have PRE_START")
  (println "All checks passed"))
'
```
  </verify>
  <done>
- Full config (mounts, env, ports, docker_args, pre_start) produces valid args
- Invalid port format causes error exit
- Non-existent mount source triggers warning
- All required flags (--rm, -it, --init) are present
  </done>
</task>
...
- build-docker-args returns vector starting with ["docker" "run" ...]
- Docker args include --rm, -it, --init
...
- Config pre_start passed as PRE_START env var
