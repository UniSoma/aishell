...
must_haves:
  truths:
    - "fs/windows? predicate is callable and returns false on Unix (true on Windows)"
...
**`get-uid` (line 28-29):** Wrap in `if (fs/windows?)` — on Windows, throw `(ex-info "UID detection not supported on Windows. See: Phase 55 (Host Identity)" {:platform :windows})`. On Unix, keep existing `(-> (p/shell {:out :string} "id" "-u") :out str/trim)`.
...
<verification>
1. Verify `fs/windows?` is callable and returns false on this Unix host:
   ```bash
   bb -e "(require '[babashka.fs :as fs]) (println (fs/windows?))"
   ```
   Expected output: `false`
2. All 3 source files load in Babashka without errors:
   ```bash
   ...
   ```
3. Grep confirms 4 `fs/windows?` guards across 3 files:
   ```bash
...
4. Run `bb aishell.clj --help` to verify the full CLI still works on Unix (no behavior change).
5. No container paths are guarded — grep for `fs/windows?` should NOT appear near `/bin/bash`, `/usr/local/bin`, `/etc/passwd`, or `/tmp/`.
</verification>

<success_criteria>
- `(fs/windows?)` predicate is callable and returns false on Unix
