...
must_haves:
  truths:
    ...
    - "aishell pi [args] dispatches to run-container and launches pi coding agent in container"
    - "aishell pi without pi installed shows user-friendly error via verify-harness-available"
  artifacts:
  ...
  key_links:
    ...
    - from: "src/aishell/cli.clj"
      ...
      via: "dispatch function routes 'pi' command to run/run-container"
      pattern: "\"pi\""
    - from: "src/aishell/run.clj"
      to: "verify-harness-available"
      via: "run-container calls verify-harness-available with 'pi' which maps to 'Pi coding agent' display name"
      pattern: "\"pi\""
---
...
<task type="auto">
  ...
  7. **dispatch function** (the main `case` expression that dispatches CLI commands): Add `"pi"` case:
     ```
     "pi" (run/run-container "pi" (vec (rest clean-args)) {:unsafe unsafe? :container-name container-name-override})
     ```
     Add after the `"gemini"` case. This follows the identical pattern used for claude/opencode/codex/gemini. Without this, `aishell pi [args]` would fall through to the unknown command handler.
  ...
  <verify>
  1. `grep "with-pi" src/aishell/cli.clj` shows multiple hits (setup-spec, handle-setup, installed-harnesses, update, dispatch)
  2. `grep "with-pi" src/aishell/state.clj` shows pi in state schema docstring
  3. `grep ":with-pi" src/aishell/cli.clj | wc -l` shows at least 8 occurrences (setup-spec, handle-setup x3, installed-harnesses, handle-update x2, dispatch)
  4. Verify `installed-harnesses` includes "pi" by checking code path
  5. `grep -A3 "with-pi.*state" src/aishell/cli.clj` confirms both the handle-setup persistence and handle-update display blocks reference :with-pi on state — verifying the "aishell update shows pi version" truth
  6. Verify the volume population guard fires when only --with-pi is set: `bb -e '(println (some #(get {:with-pi true} %) [:with-claude :with-opencode :with-codex :with-gemini :with-pi]))'` returns `true` — ensuring ensure-harness-volume triggers volume population for pi-only setups
  </verify>
  <done>
...
<task type="auto">
  ...
  2. **verify-harness-available**: Add `"pi"` to the inner `case` expression that maps harness name to display name:
     ```
     "pi" "Pi coding agent"
     ```
     Add after the `"gemini"` case in `verify-harness-available`'s internal case expression. Without this, calling `aishell pi` without pi installed throws an IllegalArgumentException instead of showing a user-friendly error.

  3. **run-container** verify-harness case: Add pi entry:
     ```
     "pi" (verify-harness-available "pi" :with-pi state)
     ```
     Add after the "gemini" case in `run-container`'s verify-harness dispatch.

  4. **run-container** container-cmd case: Add pi entry:
     ```
     "pi"
     (into ["pi"] merged-args)
     ```
  ...
  5. **check-harnesses**: Add pi to the harnesses vector:
     ```
  ...
  5. `grep '"pi"' src/aishell/run.clj` shows pi in container-cmd case and verify-harness-available display name
  ...
  8. `grep '"Pi coding agent"' src/aishell/run.clj` confirms verify-harness-available has the pi display name in its case expression
  </verify>
  <done>
