...
Output: Extended config.clj with :detection key and merge strategy, patterns.clj with custom pattern detection, core.clj with allowlist filtering.
</objective>
...
2. Add require for `[aishell.config :as config]`
...
5. Integrate in the main flow (in run.clj or wherever scan-project is called):
   - Load config
   - Get detection settings: `(get config :detection {})`
   - Pass to scan-project
   - Apply allowlist filtering to results

6. Update scan-project signature:
```clojure
(defn scan-project
...
8. Clean up test files
  ...
  <done>Custom patterns detected, allowlist filters findings, enabled:false disables detection</done>
</task>
