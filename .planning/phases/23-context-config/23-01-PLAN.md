...
must_haves:
  ...
  artifacts:
    ...
    - path: "src/aishell/run.clj"
      provides: "Updated caller passing project-dir to display-warnings"
      contains: "display-warnings"
  key_links:
    ...
    - from: "src/aishell/run.clj"
      ...
      via: "display-warnings call with project-dir"
      pattern: "display-warnings.*project-dir"
---
...
Output: gitignore.clj namespace with git check-ignore wrapper, modified core.clj that annotates high-severity findings with gitignore status, updated run.clj to pass project-dir.
</objective>
...
<task type="auto">
  ...
  <files>src/aishell/detection/core.clj, src/aishell/run.clj</files>
  <action>
**In core.clj:**
...
5. Find the call site for `display-warnings` (likely in the main shell/run flow)

6. Update the call to pass `project-dir` as the first argument:
   - Locate where `project-dir` is available (from shell command args or current directory)
   - Change `(core/display-warnings findings)` to `(core/display-warnings project-dir findings)`
...
```
  ...
  <done>High-severity findings NOT in .gitignore show "(risk: may be committed)", those in .gitignore do not, run.clj passes project-dir to display-warnings</done>
</task>
...
7. run.clj correctly passes project-dir to display-warnings
</verification>
...
- [ ] run.clj updated to pass project-dir to display-warnings
