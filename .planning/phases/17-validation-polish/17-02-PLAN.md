...
must_haves:
  ...
  key_links:
    ...
    - from: "src/aishell/run.clj:run-container"
      to: "state.edn"
      via: "compares :dockerfile-hash (state already read at line 33)"
...
<task type="auto">
  ...
  Note: State is already read by run-container at line 33: `(let [state (state/read-state)]`
  The stale check simply uses this existing `state` binding - no additional read needed.
  </action>
  <verify>
