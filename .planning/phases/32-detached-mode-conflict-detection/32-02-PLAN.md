...
**Context overrides:**
- NAME-04 ("Shell mode does not use named containers") is overridden per CONTEXT.md decision: "All containers (foreground and detached) always get named containers." Shell mode gets named containers like all other modes.
</objective>
...
5. **Call placement verification:** In run.clj, confirm `ensure-name-available!` appears AFTER the `container-name-str` let-binding AND BEFORE the `docker-args` binding (i.e., after ~L81-82 and before ~L155). Read the run-container function and verify the sequential order: container-name-str → ensure-name-available! → build-docker-args → exec/shell.
6. **End-to-end parameter wiring verification:** Run `bb -e '(require (quote aishell.docker.run)) (let [args (aishell.docker.run/build-docker-args {:project-dir "/tmp" :image-tag "test" :config nil :git-identity {} :detach true :container-name "aishell-abc-claude"})] (assert (some #{"--name"} args) "--name flag missing") (assert (some #{"aishell-abc-claude"} args) "container name value missing") (assert (some #{"--detach"} args) "--detach flag missing") (println "PASS: --name, container-name value, and --detach all present in docker args"))'` -- must print PASS confirming :container-name flows from run.clj through to docker/run.clj and into --name flag.
  </verify>
  <done>
