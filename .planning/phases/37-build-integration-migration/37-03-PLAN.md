...
Check if run.clj references `ext/build-extended-image` with `:base-tag`. If so, update to `:foundation-tag`. Looking at line 51-56 of run.clj, `build-extended-image` is called with `:base-tag`. Update this to `:foundation-tag`.
