# Milestone v2.4.0: Multi-Harness Support
...
Add OpenAI Codex CLI and Google Gemini CLI support to aishell, enabling users to run multiple AI coding harnesses in the same isolated sandbox environment.
...
- Present OAuth and API key authentication options equally
- Organize environment variables by category (harness-specific, cloud provider, other)
- Use Mermaid.js for architecture diagrams (GitHub native rendering)
- Split architecture and configuration into separate docs
- Comparison table at document start for quick harness selection
- Document container-specific auth nuances in each harness section
...
- Dynamic help based on installed harnesses (captured as todo)
- Support list format for pre_start (captured as todo)
- Audit security detection patterns (captured as todo)
