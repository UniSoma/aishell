...
updated: '2026-09-03T15:37:04.754678901Z'
...
- [ ] Plan tests cover all five platforms × script/binary install, pinned and latest, and an explicit error for unsupported pairs
- [ ] Script install on Linux upgrades in place to a working binary (manual check)
- [ ] Windows: `.bat` and old script removed, `aishell.exe` installed, `.old` dance works while running, `.old` cleaned on next start
- [ ] Checksum mismatch aborts before touching the install
- [ ] Progress bar on a TTY, size notice otherwise
- [ ] clj-kondo clean
